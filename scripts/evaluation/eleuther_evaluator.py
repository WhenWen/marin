...
class EleutherEvaluator(VllmTpuEvaluator):
    ...
    def run(self, model: ModelConfig, evals: List[str], output_path: str) -> None:
        ...
        run_bash_command(
            f"lm_eval --model local-completions --tasks {evals} --model_args model={model},base_url=http://localhost:8000/v1/completions,num_concurrent=1,max_retries=3,tokenized_requests=False,batch_size=16 --output_path {self.BENCHMARK_OUTPUT_PATH}"
        )
