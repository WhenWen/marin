...
### 2026-07-19 14:35 - d768 passes halfway with improving Paloma

- Command: refresh the exact Iris prefix, scan the complete d768 W&B train history and scheduled evaluations, read the newest temporary checkpoint metadata, and scan recent logs for non-finite, memory, task-failure, and dead-node signatures.
- Result: d768 remains running with zero failures/preemptions at step 8,370, past the halfway point. All scanned train losses remain finite; current loss is 3.179600 at 224,782.49 tokens/s. Step-8,000 Paloma macro loss is 3.527793 (BPB 1.265393), improved from 3.667298 at step 5,000. Complete temporary checkpoint metadata exists at step 8,080, timestamp `2026-07-19T21:30:21.504535`. The error scan is clean apart from the successful JAX serialization check.
- Interpretation: d768 remains numerically stable, evaluated, and recoverable through the halfway point, with no recovery action indicated.
- Next action: continue monitoring the original d768 child to terminal success and permanent step-16,875 metadata.

### 2026-07-19 15:58 - d768 passes step-12,000 evaluation
...
### 2026-07-19 17:54 - Both Gate 1 widths terminal; Identity-HC modestly beats matched July baselines

- Command: verify both Iris child summaries and the parent prefix, scan each complete W&B train history and terminal scheduled evaluation, read both permanent checkpoint metadata files, audit the terminal d768 log through W&B synchronization, and compare each terminal Paloma row against the same-width `moe_may_july_baseline` run.
- Terminal result: both children and the parent succeeded with exit code 0 and zero failures/preemptions; no restart was used. d512 finished in 1h38m34.58s with 10,979 finite W&B train rows, final train loss 3.175577, and terminal Paloma macro loss 3.544176 (BPB 1.270561). d768 finished in 6h28m35.09s with 16,874 finite W&B train rows, final train loss 2.967125, and terminal Paloma macro loss 3.208981 (BPB 1.152944). The d768 terminal log confirms evaluation, checkpoint commit, W&B synchronization, and clean completion.
- Checkpoints: permanent metadata confirms d512 step 10,980 at `gs://marin-us-east5/grug/MOE-JULY-IHC-G1-001-d512-39b35d/checkpoints/step-10980` (`2026-07-19T19:48:52.652431`) and d768 step 16,875 at `gs://marin-us-east5/grug/MOE-JULY-IHC-G1-002-d768-c9eb80/checkpoints/step-16875` (`2026-07-20T00:48:31.189927`).
- Matched Paloma comparison: the baseline and Identity-HC configs match sequence length 8,192, width/layer/head/KV-head shape, batch size, seed 0, and terminal step. At d512, macro loss improves from 3.566663 to 3.544176, delta -0.022487 (-0.6305%), and macro BPB improves from 1.278382 to 1.270561, delta -0.007821. At d768, macro loss improves from 3.227240 to 3.208981, delta -0.018259 (-0.5658%), and macro BPB improves from 1.159093 to 1.152944, delta -0.006149.
- Interpretation: the accepted four-stream Identity-HC implementation completes both real-July-baseline Gate 1 schedules without recovery, stays within the accepted matched throughput overhead (7.0423% d512; 10.2001% d768), is activation-memory viable with two-layer rematerialization, and yields a small favorable terminal Paloma result at both widths.
- Next action: publish the combined terminal result to issue #7409 and leave the issue open for the research decision.
