...
def fit_power_law(
    ...
    if len(y) == 0 or len(N) == 0 or len(D) == 0:
        raise ValueError("Cannot fit power law to empty data arrays")
    ...
    if use_log_space:
        if initial_guess is None:
            initial_guess = [0.0, 0.0, 1.0, 1.0, min_y]  # [log_A, log_B, alpha, beta, E]
        bounds = [
        ...
        if initial_guess is None:
            initial_guess = [1.0, 1.0, 1.0, 1.0, min_y]  # [A, B, alpha, beta, E]
        bounds = [
            (0, None),  # A >= 0
            (0, None),  # B >= 0
            (0, None),  # alpha >= 0
    ...
    result = minimize(
        ...
        method="L-BFGS-B",
        bounds=bounds,
        options={"ftol": OPTIMIZATION_TOLERANCE, "gtol": OPTIMIZATION_TOLERANCE, "maxiter": 15000},
    )
...
def get_default_projection_points(count_embedding_params: bool = False) -> list[ProjectionPoint]:
    ...
    token_multipliers = [0.5, 1, 5, 10, 20, 50, 100]
...
def plot_actual_vs_predicted(
    ...
    y_min = min(np.min(y_actual), np.min(y_predicted))
    y_max = max(np.max(y_actual), np.max(y_predicted)) if y_max is None else y_max
    padding = (y_max - y_min) * 0.1  # 10% padding
    plt.ylim(max(0, y_min - padding), y_max + padding)
    
    # Set y-axis ticks at 0.2 intervals
    yticks = np.arange(0, y_max + padding + 0.1, 0.1)
    plt.yticks(yticks)
...
def fit_scaling_laws(
    ...
    if len(loss_df_filtered) == 0:
        raise ValueError(f"All data was filtered out by filter_zero_d. Original data had {len(loss_df)} rows")
        
    loss_df_agg = aggregate_steps(loss_df_filtered) #, step_mode="filtered", token_filters=token_filters, tokens_col=tokens_col)
    if len(loss_df_agg) == 0:
        raise ValueError(f"All data was filtered out by aggregate_steps. Pre-aggregation data had {len(loss_df_filtered)} rows")
    ...
    if projection_points:
        ...
        for loss_metric in loss_metrics:
            if loss_metric not in loss_df_agg.columns:
                raise ValueError(f"Metric {loss_metric} not found in aggregated data. Available columns: {list(loss_df_agg.columns)}")
            y = loss_df_agg[loss_metric].values
            if len(y) == 0:
                raise ValueError(f"No data found for metric {loss_metric} in aggregated dataframe")
