...
def grugmoe_inference_state_dict(model: Transformer, prefix: str | None = None) -> dict[str, jax.Array]:
    ...
    for layer_index, block in enumerate(model.blocks):
        ...
        if block.mlp.expert_mlp.split_w_gate_up:
            gate = block.mlp.expert_mlp.w_gate
            up = block.mlp.expert_mlp.w_up
            assert gate is not None
            assert up is not None
        else:
            w_gate_up = block.mlp.expert_mlp.w_gate_up
            assert w_gate_up is not None
            gate, up = jnp.split(w_gate_up, [model.config.intermediate_dim], axis=-1)
        tensors.update(
            {
