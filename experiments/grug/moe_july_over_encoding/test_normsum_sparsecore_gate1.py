...
from experiments.grug.moe_july_over_encoding.normsum_sparsecore_gate1 import build_step
...
def test_normsum_sparsecore_gate1_matches_canonical_july_outside_oe_fields():
    july = build_july_step(hidden_dim=512, batch_size=16, num_steps=10_980).config
    ...
    assert candidate.run_id == "MOE-OE-JULY-NORMSUM-SC-GATE1-d512"
    assert model.over_encoding_vocab_size == 1_619_087
    ...
    assert candidate.batch_size.value == 16
    assert candidate.steps.value == 10_980
    ...
    assert candidate.tracker.group == "MOE-OE-JULY-normsum-sc-gate1-issue-7368"
