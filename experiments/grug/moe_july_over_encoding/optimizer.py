...
from experiments.grug.moe_july_over_encoding.adamh import scale_by_adamh
...
@OptimizerConfig.register_subclass("grug_moe_july_oe_adamh_v2")
...
@OptimizerConfig.register_subclass("grug_moe_july_oe_muonh_v1")
@dataclass(frozen=True)
class GrugMoeMuonHConfig(OptimizerConfig):
    """May Recipe MuonH optimizer with a separate OE-table Adam schedule.
    ...
    - ``adam``: ``token_embed`` / ``router`` / ``router_bias`` / ``attn_gate``
      / 1-D norm weights.
    - ``over_encoding_adam``: only ``over_encoding.tables``. It follows the
      canonical Adam schedule scaled by ``over_encoding_lr_multiplier``.
    ...
    def build(self, num_train_steps):
        ...
        over_encoding_lr_schedule = self.lr_scheduler(
            num_train_steps,
            override_lr=self.adam_lr * self.over_encoding_lr_multiplier,
        )
