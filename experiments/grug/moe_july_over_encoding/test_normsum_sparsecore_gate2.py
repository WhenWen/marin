...
from experiments.grug.moe_july_over_encoding.normsum_sparsecore_gate2 import build_step
...
def test_normsum_sparsecore_gate2_matches_canonical_july_outside_oe_fields():
    ...
    assert candidate.run_id == "MOE-OE-JULY-NORMSUM-SC-GATE2-d768"
    ...
    assert model.hidden_dim // (model.over_encoding_splits * (model.over_encoding_num_grams - 1)) == 96
    assert optimizer.over_encoding_lr_multiplier == 0.5
    assert math.isclose(optimizer.adam_lr * optimizer.over_encoding_lr_multiplier, 0.0009661103604079334)
    ...
    assert candidate.tracker.group == "MOE-OE-JULY-normsum-sc-gate2-issue-7368"
