...
"""Canonical July Baseline MoE with hierarchical Over-Encoding.
...
def _over_encoding_table_rows(logical_rows: int) -> int:
    """Pad OE tables so common power-of-two DP groups shard rows evenly."""
    return math.ceil(logical_rows / _OVER_ENCODING_TABLE_ROW_ALIGNMENT) * _OVER_ENCODING_TABLE_ROW_ALIGNMENT
...
def _rowwise_embedding_lookup_local(
    table_local: Float[Array, "Vlocal D"],
    ids_local: Int[Array, "Blocal S"],
    *,
    dp_size: int,
) -> Float[Array, "Blocal S D"]:
    """Route IDs to row owners and route the resulting embeddings back."""
    ...
    global_token_capacity = flat_ids.shape[0] * dp_size
    dispatched_ids = jax.lax.ragged_all_to_all(
        sorted_local_ids,
        jnp.zeros((global_token_capacity,), dtype=jnp.int32),
        input_offsets,
        send_sizes,
        output_offsets,
        recv_sizes,
    ...
    returned_embeddings = jax.lax.ragged_all_to_all(
    ...
    unsorted_embeddings = returned_embeddings[jnp.argsort(sort_indices)]
    return unsorted_embeddings.reshape(*ids_local.shape, table_local.shape[1])


def _rowwise_embedding_lookup(
    table: Float[Array, "V D"],
    ids: Int[Array, "B S"],
) -> Float[Array, "B S D"]:
    """Look up a row-sharded embedding table without materializing it on DP ranks."""
    ...
    def local_lookup(table_local: jax.Array, ids_local: jax.Array) -> jax.Array:
        return _rowwise_embedding_lookup_local(table_local, ids_local, dp_size=dp_size)
...
def _causal_ngram_ids(
    ...
    *,
    ...
    """Hash a causal n-gram without crossing packed-document boundaries."""
    if order < 2:
...
class Transformer(eqx.Module):
    ...
    def init(cfg: GrugModelConfig, *, key: PRNGKeyArray) -> "Transformer":
        ...
        over_encoding_key = random.fold_in(key, 0x4F45)
        token_embed = reshard(
            _init_weight(embed_key, (cfg.vocab_size, cfg.hidden_dim), cfg.initializer_std), Pembed_vocab
        )
        over_encoding = OverEncoding.init(cfg, key=over_encoding_key) if cfg.over_encoding_vocab_size > 0 else None
