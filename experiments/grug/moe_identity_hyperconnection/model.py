...
"""July-baseline MoE with Identity Hyper-Connections.

The attention and MoE sublayers retain the July baseline unchanged. Each residual
site uses four parallel streams with input-dependent non-negative read/write
weights, while the residual mapping is fixed to the identity. Consecutive blocks
are rematerialized in groups so the widened stream is not saved at every layer.
"""
...
class GrugModelConfig:
    ...
    """Number of Identity-HC residual streams. The experiment uses the paper's n=4."""
    hyperconnection_alpha_init: float = 0.01
    """Initial dynamic-mapping gates for H_pre and H_post."""
    hyperconnection_remat_layers: int = 2
    """Number of consecutive blocks per rematerialization group."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.num_residual_streams <= 0:
            raise ValueError("num_residual_streams must be positive")
        if self.hyperconnection_alpha_init < 0:
            raise ValueError("hyperconnection_alpha_init must be non-negative")
        if self.hyperconnection_remat_layers <= 0:
            raise ValueError("hyperconnection_remat_layers must be positive")
...
def _identity_hc_collapse(
    streams: jax.Array,
    mapping_projection: jax.Array,
    mapping_bias: jax.Array,
    mapping_scale: jax.Array,
    *,
    num_streams: int,
    norm_eps: float,
) -> tuple[jax.Array, jax.Array, jax.Array]:
    flat = rearrange(streams, "b s n d -> b s (n d)")
    ...
    variance = jnp.mean(jnp.square(flat.astype(jnp.float32)), axis=-1, keepdims=True)
    projected = projected * jax.lax.rsqrt(variance + norm_eps)
...
def _identity_hc_residual_update(streams: jax.Array, sublayer_output: jax.Array, post: jax.Array) -> jax.Array:
    update = post.astype(sublayer_output.dtype)[..., None] * sublayer_output[..., None, :]
    return streams + update


class IdentityHyperConnection(eqx.Module):
    """Dynamic H_pre/H_post with an exact identity residual map.

    The projection consumes all residual streams, so the learned read and write
    weights remain input dependent even though H_res is fixed to I. Projection
    and coefficient math use float32 accumulation; the stream tensors stay in
    ...
    mapping_bias: jax.Array
    mapping_scale: jax.Array
    num_streams: int = eqx.field(static=True)
    norm_eps: float = eqx.field(static=True)

    @staticmethod
    def init(cfg: GrugModelConfig, *, key: PRNGKeyArray) -> "IdentityHyperConnection":
        num_streams = cfg.num_residual_streams
        input_dim = num_streams * cfg.hidden_dim
        output_dim = 2 * num_streams
        limit = jnp.sqrt(6.0 / (input_dim + output_dim))
        projection = random.uniform(
            key,
        ...
        return IdentityHyperConnection(
            mapping_projection=reshard(projection, P(None, None)),
            mapping_bias=jnp.zeros((output_dim,), dtype=jnp.float32),
            mapping_scale=jnp.full((2,), cfg.hyperconnection_alpha_init, dtype=jnp.float32),
            num_streams=num_streams,
            norm_eps=_IDENTITY_HC_EPS,
        )

    @named_call
    def collapse(self, streams: Float[Array, "B S N D"]) -> tuple[Float[Array, "B S D"], jax.Array, jax.Array]:
        """Collapse residual streams and return H_pre/H_post coefficients."""
        collapsed, pre, post = _identity_hc_collapse(
            streams,
            reshard(self.mapping_projection, P(None, None)),
            unshard(self.mapping_bias),
            unshard(self.mapping_scale),
            num_streams=self.num_streams,
            norm_eps=self.norm_eps,
        )
        return _batch_reshard(collapsed), pre, post

    @named_call
    def add_sublayer_output(
        self,
        streams: Float[Array, "B S N D"],
        sublayer_output: Float[Array, "B S D"],
        post: jax.Array,
    ) -> Float[Array, "B S N D"]:
        """Apply H_post and the exact identity residual update."""
        return _batch_reshard(_identity_hc_residual_update(streams, sublayer_output, post))
...
class Block(eqx.Module):
    ...
    mlp_hc: IdentityHyperConnection
    rms_mlp: RMSNorm
    ...
    def init(cfg: GrugModelConfig, *, key: PRNGKeyArray) -> "Block":
        attn_key, mlp_key, shared_key, gn_attn_key, gn_mlp_key, attn_hc_key, mlp_hc_key = random.split(key, 7)
        shared = None
        ...
        return Block(
            attn_hc=IdentityHyperConnection.init(cfg, key=attn_hc_key),
            rms_attn=RMSNorm.init(cfg.hidden_dim, cfg.layer_norm_eps),
            attn_gated_norm=GatedNorm.init(cfg.hidden_dim, cfg.initializer_std, key=gn_attn_key),
            attn=CausalSelfAttention.init(cfg, key=attn_key),
            mlp_hc=IdentityHyperConnection.init(cfg, key=mlp_hc_key),
...
def _run_block_group(
    blocks: tuple[Block, ...],
    hidden: Float[Array, "B S N D"],
    masks: tuple[AttentionMask | jax.Array, ...],
    use_pko: tuple[bool, ...],
    disable_rope: tuple[bool, ...],
) -> tuple[Float[Array, "B S N D"], tuple[dict[str, jax.Array], ...]]:
    """Run one rematerialization group of Identity-HC blocks."""
    router_stats: list[dict[str, jax.Array]] = []
    for block, layer_mask, layer_use_pko, layer_disable_rope in zip(blocks, masks, use_pko, disable_rope, strict=True):
        hidden, layer_stats = block(hidden, layer_mask, layer_use_pko, layer_disable_rope)
        router_stats.append(layer_stats)
    return hidden, tuple(router_stats)
...
class Transformer(eqx.Module):
    ...
    def __call__(
        ...
        hidden = jnp.broadcast_to(
            hidden[..., None, :],
            (*hidden.shape[:-1], cfg.num_residual_streams, hidden.shape[-1]),
        )
        ...
        layer_masks: list[AttentionMask | jax.Array] = []
        layer_use_pko: list[bool] = []
        layer_disable_rope: list[bool] = []
        for i in range(num_blocks):
            is_last = i == num_blocks - 1
            is_long = i % 4 == 3 or is_last
            layer_masks.append(long_mask if is_long else short_mask)
            layer_use_pko.append(is_long and not cfg.disable_pko)
            layer_disable_rope.append(is_long and cfg.disable_long_rope)

        moe_router_stats: list[dict[str, jax.Array]] = []
        remat_layers = cfg.hyperconnection_remat_layers
        checkpointed_group = eqx.filter_checkpoint(_run_block_group, policy=remat_policy)
        for start in range(0, num_blocks, remat_layers):
            end = min(start + remat_layers, num_blocks)
            hidden, group_stats = checkpointed_group(
                self.blocks[start:end],
                hidden,
                tuple(layer_masks[start:end]),
                tuple(layer_use_pko[start:end]),
                tuple(layer_disable_rope[start:end]),
            )
            moe_router_stats.extend(group_stats)
