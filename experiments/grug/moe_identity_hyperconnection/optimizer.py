...
from experiments.grug.moe_identity_hyperconnection.adamh import scale_by_adamh
...
@OptimizerConfig.register_subclass("grug_moe_identity_hc_adamh_v1")
...
class GrugMoeAdamHConfig(OptimizerConfig):
    ...
    - adam: norms, biases, router, embeddings, attention gates, and all
      Identity-HC mapping parameters
    """
    ...
    def create_mask(self, params):
        ...
        def mask_fn(param, path):
            ...
            if ".attn_hc." in path_lower or ".mlp_hc." in path_lower:
                return "adam"
...
@OptimizerConfig.register_subclass("grug_moe_identity_hc_muonh_v1")
@dataclass(frozen=True)
class GrugMoeMuonHConfig(OptimizerConfig):
    ...
    - ``adam``: ``token_embed`` / ``router`` / ``router_bias`` / ``attn_gate``
      / 1-D norm weights / all Identity-HC mapping parameters.
