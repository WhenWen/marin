...
- Produces one PDF per model size at `experiments/optimizer_sweep/Analysis/figs/`
  named `optimizer_loss_scaling_{chinchilla}.pdf`, plotting loss vs. Model Size
...
def collect_results(results_root: str) -> pd.DataFrame:
    ...
    if not os.path.isdir(results_root):
        return pd.DataFrame(columns=["optimizer", "chinchilla", "model_size", "loss"])
...
df_loss["effective_model_size"] = df_loss["model_size"].apply(lambda x: expected_params[x])



for chinchilla in sorted(df_loss["chinchilla"].unique()):
    data = df_loss[df_loss["chinchilla"] == chinchilla]

    plt.figure()
    for optimizer in sorted(data["optimizer"].unique()):
        subset = data[data["optimizer"] == optimizer].sort_values(by="effective_model_size")
        plt.plot(
            subset["effective_model_size"],
            subset["loss"],
            # label=optimizer,
    ...
    # Labels and styling
    plt.xticks(list(expected_params.values()), list(expected_params.keys()), fontsize=18)
    plt.xlabel("Model Size", fontsize=18)
    plt.ylabel("Loss", fontsize=18)
    plt.title(f"C4/EN Loss for {chinchilla}x Chinchilla Data", fontsize=18)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(figs_dir, f"optimizer_loss_scaling_{chinchilla}.pdf"), bbox_inches="tight")
